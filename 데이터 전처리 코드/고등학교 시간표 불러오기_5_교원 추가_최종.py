...
# 학교별 교원 데이터
with open('데이터/2023_학교별 교원 현황.json', 'r', encoding='utf-8') as file:
    teachers_data = json.load(file)

# 학교별 개설 교과목 데이터
with open('데이터/2022년도_학교별 개설 교과목_clean_위치_학년통합_학기통합.json', 'r', encoding='utf-8') as file:
...
# 결과를 JSON 파일로 저장
with open('데이터/2022년도_학교별 개설교과목_교원 수.json', 'w', encoding='utf-8') as file:
    json.dump(merged_data, file, ensure_ascii=False, indent=2)

print("데이터가 성공적으로 합쳐졌고, 2022년도_학교별 개설교과목_교원 수.json 파일에 저장되었습니다.")
